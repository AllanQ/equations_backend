...
helpers do
  def authenticate!(payload)
    ...
    public_key_server = File.read('config/keys/id_rsa.pub').split(' ')[1]
    halt 403 unless public_key_client == public_key_server
    uri = request.path
    body = payload.to_s
    str = ''
    File.readlines('config/keys/id_rsa').each do |line|
      str << line if line[0] != '-'
    end
    ...
    # computed_signature = OpenSSL::HMAC
    #   .hexdigest(OpenSSL::Digest.new('sha256'), uri + body, private_key)
